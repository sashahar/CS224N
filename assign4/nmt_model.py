...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        #(in_features, out_features) = (hidden_size, 2*hidden_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, last = self.encoder(X_packed)
        last_hidden, last_cell = last
        enc_hiddens, lengths = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first = True)
        a,b,h = last_hidden.shape
        last_hidden = last_hidden.permute((1, 0, 2)).reshape((b, a*h))
        init_decoder_hidden = self.h_projection(last_hidden)
        init_decoder_cell = self.c_projection(last_cell.permute((1, 0, 2)).reshape((b, a*h)))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(enc_hiddens, alpha_t)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            contiuating_hyp_scores = (hyp_scores.unsqueeze(1).expand_as(log_p_t.unsqueeze(1)) + log_p_t.unsqueeze(1))
